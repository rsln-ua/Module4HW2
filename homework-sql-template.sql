...
SELECT * FROM Person.Person WHERE YEAR(ModifiedDate) LIKE 2008 AND MiddleName IS NOT NULL AND Title IS NULL
...
SELECT DISTINCT * FROM HumanResources.Department as d WHERE (
    SELECT COUNT(*) FROM HumanResources.EmployeeDepartmentHistory WHERE DepartmentID LIKE d.DepartmentID
) > 0
...
SELECT TerritoryID, SUM(CommissionPct) as CommisionPctSum FROM Sales.SalesPerson GROUP BY TerritoryID HAVING SUM(CommissionPct) > 0
